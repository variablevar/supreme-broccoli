...
export function yearToDateRevenue(ledger: LedgerEntry[], asOf: string) {
  ...
  const total = ledger.reduce((sum, entry) => {
    ...
    return new Date(entry.created_at).getUTCFullYear() === year &&
      entry.kind === "device_daily_reward"
      ? sum + amount
      : sum;
  }, 0n);
...
export function BalanceFlowChart({
  ...
  return (
    <Panel
      title="Balance flow"
      description={`${year} account credits and completed withdrawals by month.`}
    >
      {ledger.length ? (
        <div
          className="h-72 w-full"
          aria-label="Monthly balance inflow and outflow chart"
        >
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              <Tooltip
                formatter={(value) => [`${Number(value).toFixed(6)} USDT`]}
                contentStyle={{
        ...
        <p className="py-10 text-center text-sm text-muted-foreground">
          Balance activity will appear here.
        </p>
      )}
